...
|---|---|---|---|
| L1 unified pre-commit quality | Statements, branches, functions and lines each ≥95%; no skipped/focused/empty runs; strict types and check-only lint with zero errors/warnings; index snapshot and <30s | planned | Snapshot coverage/static subchecks are enforced: staged-source package build, typecheck, Biome, selected-run reporter and staged Gitleaks. All four scoped coverage floors are ≥95%; `allowOnly: false` and `passWithNoTests: false` reject invalid runs. Pages and some executable tooling remain outside the coverage denominator; the full gate previously took ~8 minutes on a loaded host, so the timing contract remains unmet. |

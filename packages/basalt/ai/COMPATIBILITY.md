...
3. **Form Contract & Known Limitations:** Standard inputs (`Input`, `InputArea`, `Checkbox`, `Radio`, `Switch`) participate in `FormData` extraction. Known limitations: `DatePicker` custom ref overrides are slated for complete behavior isolation; native reset and required validation boundaries are scheduled for hardening. `Autocomplete` unselected free-text and label matching blur commit preserves natural focus traversal.

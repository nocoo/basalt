...
describe("AppSidebar", () => {
	...
	it("keeps example pages and lists every library export", { timeout: 45_000 }, () => {
		...
		expect(
			screen
				...
				.compareDocumentPosition(screen.getAllByRole("button", { name: "Loading" })[1]) & following,
		).toBeTruthy();
		expect(
			screen
				.getAllByRole("button", { name: "Loading" })[1]
				.compareDocumentPosition(screen.getByRole("button", { name: "Animation" })) & following,
		).toBeTruthy();
		expect(
			screen
		...
		expect(
			screen
				...
				.compareDocumentPosition(screen.getByRole("button", { name: "Chat" })) & following,
		).toBeTruthy();
		expect(
			screen
	...
	it("disables exactly the planned sidebar entries without changing ready navigation", () => {
		...
		expect(catalogButtons).toHaveLength(112);
		expect(catalogButtons.filter((button) => !button.disabled)).toHaveLength(111);
	...
	it("applies the same planned gate in the command palette", async () => {
		...
		expect(catalogOptions).toHaveLength(112);
		expect(
	...
	it("does not show pending maturity badges in the components sidebar", () => {
		...
		expect(buttons).toHaveLength(112);

...
describe("ui catalog", () => {
	...
	it("renders the categorized index with orthogonal release and page states", () => {
		...
		expect(document.querySelector("[data-ready-summary]")).toHaveTextContent("101 / 102 ready");
		...
		for (const [index, group] of CATALOG_INDEX_GROUPS.entries()) {
			...
			expect(section.querySelectorAll("[data-catalog-card]")).toHaveLength([74, 25, 3][index]);
		}
		expect(document.querySelectorAll("[data-catalog-card]")).toHaveLength(102);
	...
	it("canonicalizes invalid and repeated owned URL values without removing foreign values", async () => {
		...
		expect(document.querySelector("[data-result-summary]")).toHaveTextContent("101 results");
		await waitFor(() => {
			expect(document.querySelector("[data-router-location]")).toHaveAttribute(
				"data-router-location",
	...
	it("renders one empty state and resets owned filters while preserving foreign values", () => {
		...
		expect(document.querySelector("[data-result-summary]")).toHaveTextContent("102 results");
		expect(document.querySelectorAll("[data-catalog-card]")).toHaveLength(102);
	...
	it("sources layer-card API rows from generated catalog data", async () => {
		...
		expect(api).toHaveTextContent("LayerCard.Well");

  ...
  return (
    <DashboardLayout title="Investments" currentPath="/investments">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Portfolio Value</p>
          <h2 className="text-2xl font-semibold text-foreground">${totalValue.toLocaleString()}</h2>
        ...
        <div className="rounded-[14px] bg-secondary p-5">
          ...
          <h2 className="text-2xl font-semibold text-success">+$342.50</h2>
          <span className="text-xs font-medium text-success">+0.34%</span>
        </div>
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Total Return</p>
          <h2 className="text-2xl font-semibold text-foreground">$8,600</h2>
          ...
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={performanceData}>
                ...
                <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 12 }} axisLine={false} tickLine={false} width={45} tickFormatter={(v: number) => `$${(v/1000).toFixed(0)}k`} />
        ...
        <div className="rounded-[14px] bg-secondary p-5">
          ...
          <div className="flex items-center gap-6">
            <div className="h-[180px] w-[180px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={portfolio} cx="50%" cy="50%" innerRadius={50} outerRadius={75} dataKey="allocation" strokeWidth={0}>

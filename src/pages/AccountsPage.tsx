        ...
        {cards.map((card) => (
          <div key={card.name} className="rounded-card bg-secondary p-4">
            ...
            <p className="text-[11px] text-muted-foreground mt-1.5">
              ${card.balance.toLocaleString()} / ${card.limit.toLocaleString()} {t("pages.accounts.limit")}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-card bg-secondary p-5">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          {[
            ...
            <div key={feat.key} className="flex items-center justify-between rounded-widget bg-card p-3">

...
function ChipIcon({ isBlack }: { isBlack: boolean }) {
	return (
		<div
			className={`h-8 w-10 rounded-md ${isBlack ? "bg-amber-300/80" : "bg-amber-200/80"} flex items-center justify-center`}
		>
			<div
				className={`h-5 w-7 rounded-xs border ${isBlack ? "border-amber-600/50" : "border-amber-400/60"} grid grid-cols-3 grid-rows-2 gap-px p-px`}
			>
				{Array.from({ length: 6 }).map((_, i) => (
					<div
						key={i}
						className={`${isBlack ? "bg-amber-500/40" : "bg-amber-300/50"} rounded-[1px]`}
					/>
				))}
			</div>
		</div>
	...
	return (
		<div className="space-y-4">
				...
				{cards.map((card) => {
					...
					return (
						...
						>
							{/* Decorative circles */}
							<div
								className={`absolute -top-12 -right-12 h-40 w-40 rounded-full ${cs.overlayOpacity.large}`}
							/>
							<div
								className={`absolute -bottom-8 -left-8 h-32 w-32 rounded-full ${cs.overlayOpacity.small}`}
							/>
							...
							<div className="relative z-10 space-y-3">
								...
								<p className={`text-sm font-mono tracking-[0.2em] ${cs.textSecondary}`}>
									{card.number}
								</p>
							</div>
							...
							<div className="flex items-end justify-between relative z-10">
								<div className="flex gap-6">
									<div>
										<p className={`text-[9px] uppercase ${cs.textMuted}`}>
				...
				{cards.map((card) => (
					<div key={card.name} className="rounded-card bg-secondary p-4">
						<div className="flex items-center justify-between mb-2">
							...
							<span className="text-xs text-muted-foreground">{card.utilization}%</span>
						</div>
						<div
					...
					{[
						...
						>
							...
							<Switch
								...
								className="h-5 w-9 data-[state=checked]:bg-success/20 data-[state=unchecked]:bg-input [&>span]:h-4 [&>span]:w-4 [&>span]:data-[state=checked]:bg-success [&>span]:data-[state=checked]:translate-x-4"
							/>
						</div>
					))}
				</div>
					...
					{activityList.map((item, i) => (
						<div key={i} className="flex items-center justify-between py-1">
							<div className="flex items-center gap-3">
								<div
									className={`flex h-8 w-8 items-center justify-center rounded-lg ${item.direction === "positive" ? "bg-success/10" : "bg-destructive/10"}`}
								>
							...
							<span
								className={`text-sm font-medium ${item.direction === "positive" ? "text-success" : "text-foreground"}`}
							>
			...
			<div className="flex items-center justify-between">
				...
				<button
					type="button"
					className="flex items-center gap-2 rounded-widget bg-secondary px-3 py-2 text-sm text-muted-foreground hover:text-foreground"
				>
			...
			<div className="flex flex-col gap-2 md:hidden">
				{records.map((tx) => (
					<div key={tx.id} className="rounded-card bg-secondary p-4">
						<div className="flex items-center gap-3">
							<div
								className={`flex h-8 w-8 items-center justify-center rounded-lg shrink-0 ${tx.direction === "positive" ? "bg-success/10" : "bg-destructive/10"}`}
							>
								{tx.direction === "positive" ? (
									<ArrowDownLeft className="h-3.5 w-3.5 text-success" strokeWidth={1.5} />
								) : (
							...
							<div className="text-right shrink-0">
								<p
									className={`text-sm font-medium ${tx.direction === "positive" ? "text-success" : "text-foreground"}`}
								>
									{tx.formattedAmount}
								</p>
								<span
									className={`text-[10px] ${tx.statusVariant === "success" ? "text-success" : "text-yellow-500"}`}
								>
									{tx.status}
								</span>
							</div>

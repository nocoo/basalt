...
describe("catalog index model", () => {
	it("groups every catalog entry exactly once", () => {
		...
		expect(CATALOG_INDEX_GROUPS.map((group) => group.items.length)).toEqual([78, 25, 3]);
		expect(CATALOG_INDEX_ITEMS).toHaveLength(106);
	...
	it("models the current page and release states independently", () => {
		expect(CATALOG_INDEX_READY_COUNT).toBe(105);
		expect(
	...
	it("models all public catalog navigation as 93 ready and maps planned", () => {
		...
		expect(states.filter((item) => item.pageStatus === "ready")).toHaveLength(105);

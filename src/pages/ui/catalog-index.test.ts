...
describe("catalog index model", () => {
	it("groups every non-doc catalog entry exactly once", () => {
		...
		expect(CATALOG_INDEX_GROUPS.map((group) => group.items.length)).toEqual([62, 24, 3]);
		expect(CATALOG_INDEX_ITEMS).toHaveLength(89);

		const slugs = CATALOG_INDEX_ITEMS.map((item) => item.entry.slug);
		expect(new Set(slugs).size).toBe(89);
	...
	it("models the current page and release states independently", () => {
		expect(CATALOG_INDEX_READY_COUNT).toBe(86);
		expect(
	...
	it("models all public catalog navigation as 86 ready and 12 planned pages", () => {
		const states = CATALOG.map((entry) => ({
			slug: entry.slug,
			pageStatus: resolveCatalogPageState(entry.slug, catalogDocs, catalogHero).pageStatus,
		}));
		expect(states.filter((item) => item.pageStatus === "ready")).toHaveLength(86);

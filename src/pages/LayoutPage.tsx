...
function Block({ children, className = "" }: { children?: React.ReactNode; className?: string }) {
	return (
		<div
			className={`rounded-widget border border-border bg-card p-4 text-sm text-muted-foreground ${className}`}
		>
			{children}
		</div>
	);
...
function LabeledBlock({ label, className = "" }: { label: string; className?: string }) {
	return (
		<div
			className={`rounded-widget border border-border bg-card p-4 flex items-center justify-center text-xs text-muted-foreground font-mono ${className}`}
		>
			{label}
		</div>
	);
	...
	return (
		<div className="space-y-4">
			...
			<Section title={t("pages.layout.responsiveBreakpoints")} icon={Maximize2}>
				<p className="text-xs text-muted-foreground mb-3">{t("pages.layout.responsiveDesc")}</p>
			...
			<Section title={t("pages.layout.autoFitGrid")} icon={Grid3X3}>
				<p className="text-xs text-muted-foreground mb-3">{t("pages.layout.autoFitDesc")}</p>
				<div

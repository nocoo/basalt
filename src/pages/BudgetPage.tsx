    ...
    <DashboardLayout title="Budget" currentPath="/budget">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-[14px] bg-secondary p-5">
          ...
          <h2 className="text-2xl font-semibold text-foreground">${totalLimit.toLocaleString()}</h2>
        </div>
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Spent So Far</p>
          <h2 className="text-2xl font-semibold text-foreground">${totalSpent.toLocaleString()}</h2>
        </div>
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Remaining</p>
          <h2 className="text-2xl font-semibold text-success">${(totalLimit - totalSpent).toLocaleString()}</h2>
        </div>
      </div>
        ...
        <div className="h-[200px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlyData} barGap={4}>
              ...
              <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 12 }} axisLine={false} tickLine={false} width={35} />

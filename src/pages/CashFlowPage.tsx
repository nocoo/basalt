    ...
    <DashboardLayout title="Cash Flow" currentPath="/cash-flow">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-[14px] bg-secondary p-5">
          ...
          <h2 className="text-2xl font-semibold text-success">${totalIn.toLocaleString()}</h2>
        </div>
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Total Outflow</p>
          <h2 className="text-2xl font-semibold text-destructive">${totalOut.toLocaleString()}</h2>
        </div>
        <div className="rounded-[14px] bg-secondary p-5">
          <p className="text-sm text-muted-foreground mb-1">Net Cash Flow</p>
          <h2 className="text-2xl font-semibold text-foreground">${(totalIn - totalOut).toLocaleString()}</h2>
        </div>
      </div>
        ...
        <div className="h-[240px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={monthlyFlow}>
              ...
              <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 12 }} axisLine={false} tickLine={false} width={40} />
              <defs>

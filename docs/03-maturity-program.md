...
> 状态：执行中  
> 当前切片：S2A3 D031 — source-backed LinkButton scenarios；待 Grok 实现
> 当前代码前置：`a20c691`（D030 验收台账已提交）
...
| D031 | S2A3 | `a20c691` | `w14:p1` | 执行中 | — | 只把 LinkButton 的两个生效 inline scenarios 迁入独立 source-backed modules，并移除 BASE/KUMO 同 slug 静默覆盖；Button examples、组件 API、generated docs 与其它重合 owner 全部冻结。 |

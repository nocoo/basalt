...
> 状态：执行中  
> 当前切片：S2A8 D036 — Label API type source + JSDoc default；待 Grok 实现
> 当前代码前置：`9dfffd2`（D035 验收台账已提交）
...
| S2A | 类型驱动的 docs/API/scenario 数据模型 | 执行中（S2A1 D028、S2A2 D030、S2A3 D031、S2A4 D032、S2A5 D033、S2A6 D034、S2A7 D035 完成） | 组件类型、API 表、example 不再三份手写漂移 |
...
| D036 | S2A8 | `9dfffd2` | `w14:p1` | 执行中 | — | 以通用 JSDoc `@default` 支持无损生成 Label 两项专属 API，保持默认/说明并删除 docs 手写 inventory；冻结 runtime、scenarios 与后续阶段。 |

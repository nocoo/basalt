...
> 状态：执行中  
> 当前切片：S1C1h — D026 Banner fallback 覆盖率恢复
> 已验收代码基线：`fed681831ff6`（`main`；D025 AppHeader 可选区域覆盖率完成）
...
| S1C | coverage、publint、prepublishOnly、Husky/browser 门 | 执行中（S1C1h D026） | 95% 四项 coverage 恢复；一条发布前命令覆盖所有门，仍不实际 publish |
...
| D026 | S1C1h | `fed681831ff6` + 本次调度文档 commit | `w14:p1` | 执行中 | 待提交 | 只扩充 Banner 既有测试，覆盖 null variant/size fallback 与 compact 无 description 的普通 inline action；禁止生产、coverage 配置、其它测试或 publint 变更 |

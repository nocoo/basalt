...
> 状态：执行中  
> 当前切片：S1C1d — D022 SidebarIconItem/SidebarUser 行为覆盖率恢复
> 已验收代码基线：`9f3b8d696b27`（`main`；D021 StatCard/StatGrid 行为覆盖率完成）
...
| S1C | coverage、publint、prepublishOnly、Husky/browser 门 | 执行中（S1C1d D022） | 95% 四项 coverage 恢复；一条发布前命令覆盖所有门，仍不实际 publish |
...
| D022 | S1C1d | `9f3b8d696b27` + 本次调度文档 commit | `w14:p1` | 执行中 | 待提交 | 只扩充 Sidebar 既有测试，覆盖 IconItem 默认/active 与 SidebarUser email/slots 可选状态；禁止生产、coverage 配置、其它测试或下一组件变更 |

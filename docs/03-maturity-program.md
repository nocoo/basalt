...
> 状态：执行中  
> 当前切片：S1C1g — D025 AppHeader 可选区域覆盖率恢复
> 已验收代码基线：`d9ae8a406251`（`main`；D024 Timeline 变体覆盖率完成）
...
| S1C | coverage、publint、prepublishOnly、Husky/browser 门 | 执行中（S1C1g D025） | 95% 四项 coverage 恢复；一条发布前命令覆盖所有门，仍不实际 publish |
...
| D025 | S1C1g | `d9ae8a406251` + 本次调度文档 commit | `w14:p1` | 执行中 | 待提交 | 只扩充 app-shell 既有测试，覆盖 AppHeader 无 breadcrumbs/title/actions 的三个 false 路径；禁止生产、coverage 配置、其它测试或 Banner 变更 |

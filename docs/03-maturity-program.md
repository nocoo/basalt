...
> 状态：执行中  
> 当前切片：S1B4 — D018-R3 heavy consumer parser/binding 真实性返工
> 已验收代码基线：`23346190f8ee`（`main`；`ee4a680902fa` + `09e356a3c07f` + `fee140df34af` 待返工验收）
...
| S1B | 仓外 Vite/Next/heavy granular tarball consumers | 执行中（S1B4 D018-R3） | A/B/C/D 门不使用 workspace alias 或根 node_modules 泄漏 |
...
| D018 | S1B4 | `23346190f8ee` + 本次返工调度文档 commit | `w14:p1` | 验收中（R3） | `ee4a680902fa` + `09e356a3c07f` + `fee140df34af`（待返工） | 首提交建立 D，R1 关闭 specifier/cleanup 缺口，R2 改用 SWC TSX AST 并关闭注释伪 import 与 side-effect/local-function 假阳性；但 R2 从 Vite plugin 借用未直接声明的 `@swc/core`，且名字集合不证明 JSX 引用的是 imported binding：三个解构参数同名的嵌套 JSX 可让完全未使用的 heavy imports 过门。R3 只补 parser 所有权和 binding identity，不进入 S1C |

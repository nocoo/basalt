...
> 状态：执行中  
> 当前切片：S1B3b — 仓外 Next + React 19 browser hydration（准备下发）
> 已验收代码基线：`3d993c0559f4`（`main`）
...
| 外部消费者 | 仓外 Vite Tailwind、Vite standalone、Next build/start tarball 门全绿；Next browser、optional-peer 待办 | 仓外 Vite Tailwind、Vite standalone、Next consumer 全绿 |
...
| S1B | 仓外 Vite/Next tarball consumers | 执行中（S1B3b 准备下发） | A/B/C/D 门不使用 workspace alias 或根 node_modules 泄漏 |
...
| D017 | S1B3b | `3d993c0559f4` + 本次调度文档 commit | `w14:p1` | 准备下发 | — | 只给 Gate C 增加真实 Playwright Chromium hydration、Button/Theme/Toast portal、错误捕获与双向清理证明；不进入通用组件 browser suite、S1B4 或 S1C |
